...
export default function CourriersSortantsPage() {
  ...
  const sortants = useMemo(() => {
    const q = search.toLowerCase();

    return courriers
      .filter((courrier) => courrier.type === "Sortant")
      .filter((courrier) => {
        return (
  ...
  const totalSortants = sortants.length;
  const totalEnCours = sortants.filter((c) => c.statut === "En cours").length;
  const totalTraites = sortants.filter((c) => c.statut === "Traité").length;
  const totalExpires = sortants.filter((c) => {
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold">Courriers sortants</h1>
            <p className="mt-2 text-sm text-slate-500">
              Liste complète des courriers sortants.
            </p>
          </div>
        ...
        <section className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm text-slate-500">Total sortants</p>
            <p className="mt-3 text-3xl font-bold">{totalSortants}</p>
          </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <h2 className="text-xl font-bold">Liste des courriers sortants</h2>

            <input
              type="text"
              placeholder="Rechercher par référence, objet, destinataire..."
              value={search}
        ...
        >
          ...
          >
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-2xl font-bold">Détails du courrier sortant</h2>
              <button
                onClick={() => setSelectedCourrier(null)}
